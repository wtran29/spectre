...
// Len returns a length of items in schedule as a helper to sort by host
func (a ByHost) Len() int {
	return len(a)
}

// Less is used to compare if an item in schedule is less than other as a helper to sort by host
func (a ByHost) Less(i, j int) bool {
	return a[i].Host < a[j].Host
}

// Swap is used to swap items in schedule as a helper to sort by host
